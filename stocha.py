...
for step in range(100):
	...
	print(np.dot(mx.T * mx[i], mw))
	# sigma += square(((mx[i].T * mx * mw - mx[i].T * my) * 2 / len(x)).tolist())
	mw -= mx[i] * (eta / len(x) * 2 * (np.dot(mx.T * mx[i], mw) - np.dot(mx.T[i], my)))
	# mw -= mx[i] * (eta / (step + 1) ** (1/2) * (mx.T[i] * mx * mw - mx.T[i] * my) / (sigma / (step + 1)) ** (1/2))
	etout = 0
